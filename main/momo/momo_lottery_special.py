...
def main():
    for i in range(len(cookies)):
        ...
        # 抽
        lottery="""{
            "m_promo_no" : "U95040800002",
            "doAction" : "lottery",
            "dt_promo_no" : "D95040800001"
        }
        """
        print(i+1)
        lottery_r = requests.post('https://event.momoshop.com.tw/game/momoLotterySpecial.PROMO',headers=headers1,json=json.loads(lottery))
        print(lottery_r.text)
...
#             }
#             """
#             share_r = requests.post('https://event.momoshop.com.tw/game/collectSameCardsAdd.PROMO',headers=headers1,json=json.loads(share))
#             print(share_r.text)
#             print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
#
#     for i in range(len(employeeIDs)):
...
#             """
#             share_click_r = requests.post('https://event.momoshop.com.tw/game/collectSameCardsAdd.PROMO',headers=headers1,json=json.loads(share_click))
...
#             """
#             print(i+1)
#             exchange_r = requests.post('https://event.momoshop.com.tw/game/collectSameCardsAdd.PROMO',headers=headers1,json=json.loads(exchange))
#             print(exchange_r.text)
#             print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f"))
...
#     t4 = datetime.datetime(int(years),int(months),int(today),14,59,59,900000)     #搶折扣的時間     *****檢查*****
    t4 = datetime.datetime(int(years),int(months),int(today),15,1,59,900000)     #搶折扣的時間     *****檢查*****
#     t4 = datetime.datetime(int(years),int(months),int(today),16,59,59,900000)     #搶折扣的時間     *****檢查*****
#     t4 = datetime.datetime(int(years),int(months),int(today),17,59,59,900000)     #搶折扣的時間     *****檢查*****
#     t4 = datetime.datetime(int(years),int(months),int(today),19,59,59,900000)     #搶折扣的時間     *****檢查*****
#     t4 = datetime.datetime(int(years),int(months),int(today),21,59,59,900000)     #搶折扣的時間     *****檢查*****
    print(t4)
...
while datetime.datetime.now() < t4:
    ...
    time.sleep(0.1)
    continue
if (datetime.datetime.now() >= t4): main()
