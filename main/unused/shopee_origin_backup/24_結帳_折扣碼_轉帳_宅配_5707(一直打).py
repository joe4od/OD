...
def main():
    info = json.loads(myheader.get_data_with_coupon(str(shop['unit_price']), str(shop['merchandise_subtotal']), str(shop['promocode_applied']), str(shop['total_payable']), str(shop['quantity']), str(shop['shopid']), str(shop['itemid']), str(shop['modelid']), "\""+str('RCYQ978DMT')+"\"", str('572665170100224'), str(timestamps),str(shop['voucher_type'])))
