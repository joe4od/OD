...
#voucher_code: BAPFFAHV82
#promotion_id: 742954751033344
...
def main():
#     promotion_string = get_vouchers_by_codes(voucher_special_12,myheader.headers)
    info = json.loads(myheader.get_data_with_coupon(str(shop['unit_price']), str(shop['merchandise_subtotal']), str(shop['promocode_applied']), str(shop['total_payable']), str(shop['quantity']), str(shop['shopid']), str(shop['itemid']), str(shop['modelid']), "\""+str('BAPFFAHV82')+"\"", str('742954751033344'), str(timestamps),str(shop['voucher_type'])))
